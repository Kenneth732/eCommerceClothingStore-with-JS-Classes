class ClothingStore {
  ...
  renderClothingItems() {
    try {
      ...
      console.error('Error rendering clothing items:', error);
    }
  }

  // handle Display logic and add item to cart
  async renderClothingItem(item) {
    try {
      ...
      console.error(`Error occurred when trying to display: ${error.message}`);
    }
  }
  ...
  // Update the cart every time user made change
  updateCart() {
